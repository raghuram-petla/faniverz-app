jest.mock('@/theme', () => ({
  useTheme: () => ({
    ...
    colors: { gray500: '#6b7280', red600: '#dc2626' },
  }),
}));
...
describe('CommentsList', () => {
  it('shows loading indicator when loading', () => {
    const { toJSON } = render(<CommentsList comments={[]} userId={null} isLoading={true} />);
    const json = JSON.stringify(toJSON());
    expect(json).toContain('ActivityIndicator');
  });

  it('shows empty state when no comments', () => {
    render(<CommentsList comments={[]} userId={null} isLoading={false} />);
    expect(screen.getByText('No comments yet. Be the first!')).toBeTruthy();
  });

  it('renders comments', () => {
    const comments = [makeComment('c1', 'u1'), makeComment('c2', 'u2')];
    render(<CommentsList comments={comments} userId="u1" isLoading={false} />);
