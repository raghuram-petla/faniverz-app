...
import { badRequest, errorResponse } from '@/lib/sync-helpers';
 ...
 */
// @boundary: admin-only route — wrapped by withMutationAdmin (rejects viewers + unauthenticated)
...
export const POST = withMutationAdmin('manage-user', async ({ req }) => {
  const body = await req.json();
  const { action, userId } = body;

  if (!action || !userId) {
    return badRequest('Missing action or userId');
  }
  ...
  return errorResponse('manage-user', new Error(`Unknown action: ${action}`), 400);
});
