...
const BUCKET = 'faniverz-production-house-logos';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const baseUrl = process.env.R2_PUBLIC_BASE_URL_PRODUCTION_HOUSES;
    if (!baseUrl) {
      return NextResponse.json(
        { error: 'R2_PUBLIC_BASE_URL_PRODUCTION_HOUSES is not configured.' },
        { status: 503 },
      );
    }
    ...
    console.error('Production house logo upload failed:', err);
