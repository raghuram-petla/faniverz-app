          ...
          {invitations?.map((inv) => (
            <tr key={inv.id} className="border-b border-outline-subtle hover:bg-surface-elevated">
              ...
              <td className="px-6 py-4">
                ...
                <span className="inline-block px-2.5 py-1 rounded-full text-xs font-medium bg-red-600/10 text-status-red">
                  {ADMIN_ROLE_LABELS[inv.role_id]}
                </span>
              </td>
              <td className="px-6 py-4">
                {inv.status === 'pending' && (
                  <span className="inline-flex items-center gap-1 text-status-yellow text-xs font-medium">
                ...
                {inv.status === 'revoked' && (
                  <span className="inline-flex items-center gap-1 text-status-red text-xs font-medium">
