...
import { POST } from '@/app/api/upload/production-house-logo/route';
...
describe('POST /api/upload/production-house-logo', () => {
  ...
  it('is configured with the correct bucket', () => {
    expect(mockHandler._config.bucket).toBe('faniverz-production-house-logos');
  });
  ...
  it('is configured with the correct base URL env var', () => {
    expect(mockHandler._config.baseUrlEnvVar).toBe('R2_PUBLIC_BASE_URL_PRODUCTION_HOUSES');
  });

  it('is configured with the correct label', () => {
    expect(mockHandler._config.label).toBe('Production house logo');
  });
