...
vi.mock('@/lib/image-resize', () => ({
  POSTER_VARIANTS: [{ suffix: '_sm', width: 342 }],
}));

import { POST } from '@/app/api/upload/movie-poster/route';
...
describe('POST /api/upload/movie-poster', () => {
  ...
  it('is configured with the correct bucket', () => {
    expect(mockHandler._config.bucket).toBe('faniverz-movie-posters');
  });

  it('is configured with POSTER_VARIANTS', () => {
    expect(mockHandler._config.variants).toEqual([{ suffix: '_sm', width: 342 }]);
  });

  it('is configured with the correct base URL env var', () => {
    expect(mockHandler._config.baseUrlEnvVar).toBe('R2_PUBLIC_BASE_URL_POSTERS');
  });

  it('is configured with the correct label', () => {
    expect(mockHandler._config.label).toBe('Movie poster');
  });
