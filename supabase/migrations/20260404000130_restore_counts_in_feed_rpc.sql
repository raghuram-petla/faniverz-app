-- Restore view_count, comment_count, and bookmark_count to get_personalized_feed RPC.
--
-- These columns were accidentally dropped when the RPC was rewritten in
-- 20260330000116_add_poster_image_type_to_feed_rpc.sql. The client maps
-- them in fetchPersonalizedFeed() but they returned undefined, causing
-- comment_count (and view_count) to always show 0.
...
BEGIN
  RETURN QUERY
  WITH user_genres AS MATERIALIZED (
  ...
  SELECT
    ...
    nf.view_count, nf.comment_count, nf.bookmark_count,
