-- Fix propagate_movie_poster_to_feed trigger overwriting backdrop feed entries
--
-- Bug: when movies.poster_url changes (e.g. admin sets main poster), the
-- propagation trigger overwrites ALL non-YouTube feed entries for that movie
-- with the poster URL — including backdrop entries that have their own image_url.
-- The client then resolves the poster key from the BACKDROPS CDN bucket,
-- which either shows the wrong image or fails to load entirely.
--
-- Fix:
--   1. Scope the propagation trigger to only update feed entries NOT sourced
--      from movie_images (those already have correct thumbnail_url from
--      sync_movie_poster_to_feed trigger).
--   2. Repair existing corrupted thumbnail_urls.
--   3. Fix 'posters' filter regression in get_personalized_feed RPC
--      (migration 109 dropped 'backdrop' from the filter).

-- ============================================================
-- 1. Fix propagation trigger — skip movie_images-sourced entries
-- ============================================================
...
BEGIN
  IF OLD.poster_url IS DISTINCT FROM NEW.poster_url THEN
    -- @contract: only update feed entries that use the movie poster as their
    -- thumbnail (update-type: new_movie, ott_release, etc.). Entries sourced
    -- from movie_images have their own image_url set by sync_movie_poster_to_feed.
    ...
    WHERE movie_id = NEW.id
      ...
      AND source_table IS DISTINCT FROM 'movie_images';
  END IF;
  RETURN NEW;
END;
$$;
...
-- ============================================================
-- 2. Data fix — repair corrupted thumbnail_urls
-- ============================================================
-- Restore thumbnail_url to the source movie_images.image_url for any
-- feed entry where the propagation trigger overwrote it.
...
-- ============================================================
-- 3. Fix 'posters' filter in get_personalized_feed RPC
-- ============================================================
-- Migration 109 regressed the filter to exclude backdrop feed entries.
-- Restore IN ('poster', 'backdrop') to match fetchNewsFeed client behavior.
