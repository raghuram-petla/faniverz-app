-- Add prefix matching to search_movies, search_actors, search_production_houses.
-- Problem: plainto_tsquery('nanna') does not match tsvector token 'nannaku' — they are
-- different words. User expects partial-word search (typing "nanna" finds "Nannaku Prematho").
-- Fix: build a second tsquery using to_tsquery with :* (prefix) operator so each typed
-- word matches any token that STARTS with those characters.
-- Both tsqueries (exact + prefix) are OR'd in the FTS CTE; the higher score wins.
...
BEGIN
  ...
  -- Exact word match (handles full words like "nannaku prematho")
  tsq := plainto_tsquery('pg_catalog.english', term);

  -- @edge: prefix match — strips non-alphanumeric chars, appends :* to each word
  -- so "nanna" → 'nanna:*' which matches tsvector token 'nannaku', 'nannakudu', etc.
  ...
  WITH fts AS (
    SELECT m.id,
           -- @contract: take the higher score from exact vs prefix match
           greatest(
