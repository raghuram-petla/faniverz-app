-- Fix: drop the incorrectly-created uuid-signature overload, then update the
-- original text-signature function to allow movie_images instead of movie_posters.

-- Drop the wrong overload (p_id uuid) that was accidentally created
DROP FUNCTION IF EXISTS public.admin_crud(uuid, text, text, jsonb, uuid, jsonb);

-- Replace the original function (p_id text) with movie_images in the allowlist
