-- Add movie_poster_image_type to get_personalized_feed RPC return type.
--
-- When a movie has no portrait poster and uses a backdrop as its poster
-- (poster_image_type = 'backdrop'), the client must know which R2 bucket
-- to fetch from. Without this field, the client always assumes POSTERS
-- bucket, causing 404s or showing wrong images from a different bucket.
