-- Personalized Feed: voting, auto-event triggers, scoring RPC
-- Adds upvote/downvote system, auto-generated feed events for movie lifecycle,
-- and a personalized scoring function for ranked feed delivery.

-- ============================================================
-- 1. FEED VOTES TABLE
-- ============================================================
CREATE TABLE feed_votes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  feed_item_id uuid NOT NULL REFERENCES news_feed(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  vote_type text NOT NULL CHECK (vote_type IN ('up', 'down')),
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (feed_item_id, user_id)
);

ALTER TABLE feed_votes ENABLE ROW LEVEL SECURITY;

CREATE INDEX idx_feed_votes_item ON feed_votes (feed_item_id, vote_type);
CREATE INDEX idx_feed_votes_user ON feed_votes (user_id, feed_item_id);

-- RLS: users manage own votes, everyone can read (for showing counts)
CREATE POLICY "Anyone can read feed votes"
  ON feed_votes FOR SELECT USING (true);

CREATE POLICY "Users can insert own votes"
  ON feed_votes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own votes"
  ON feed_votes FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own votes"
  ON feed_votes FOR DELETE
  USING (auth.uid() = user_id);

-- ============================================================
-- 2. NEW COLUMNS ON NEWS_FEED
-- ============================================================
ALTER TABLE news_feed ADD COLUMN upvote_count integer NOT NULL DEFAULT 0;
ALTER TABLE news_feed ADD COLUMN downvote_count integer NOT NULL DEFAULT 0;

-- ============================================================
-- 3. TRIGGER: AUTO-UPDATE VOTE COUNTS
-- ============================================================
CREATE OR REPLACE FUNCTION update_feed_vote_counts()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.vote_type = 'up' THEN
      UPDATE public.news_feed SET upvote_count = upvote_count + 1 WHERE id = NEW.feed_item_id;
    ELSE
      UPDATE public.news_feed SET downvote_count = downvote_count + 1 WHERE id = NEW.feed_item_id;
    END IF;
    RETURN NEW;
  END IF;

  IF TG_OP = 'DELETE' THEN
    IF OLD.vote_type = 'up' THEN
      UPDATE public.news_feed SET upvote_count = GREATEST(0, upvote_count - 1) WHERE id = OLD.feed_item_id;
    ELSE
      UPDATE public.news_feed SET downvote_count = GREATEST(0, downvote_count - 1) WHERE id = OLD.feed_item_id;
    END IF;
    RETURN OLD;
  END IF;

  IF TG_OP = 'UPDATE' AND OLD.vote_type IS DISTINCT FROM NEW.vote_type THEN
    -- Vote flipped: decrement old, increment new
    IF OLD.vote_type = 'up' THEN
      UPDATE public.news_feed
      SET upvote_count = GREATEST(0, upvote_count - 1),
          downvote_count = downvote_count + 1
      WHERE id = NEW.feed_item_id;
    ELSE
      UPDATE public.news_feed
      SET downvote_count = GREATEST(0, downvote_count - 1),
          upvote_count = upvote_count + 1
      WHERE id = NEW.feed_item_id;
    END IF;
    RETURN NEW;
  END IF;

  RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE TRIGGER trg_feed_vote_counts
  AFTER INSERT OR UPDATE OR DELETE ON feed_votes
  FOR EACH ROW EXECUTE FUNCTION update_feed_vote_counts();

-- ============================================================
-- 4. AUTO-EVENT TRIGGERS
-- ============================================================

-- 4a. New movie announced → feed item
CREATE OR REPLACE FUNCTION sync_new_movie_to_feed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  INSERT INTO public.news_feed (
    feed_type, content_type, title, description, movie_id,
    source_table, source_id, thumbnail_url, published_at
  ) VALUES (
    'update', 'new_movie',
    'New Movie: ' || NEW.title,
    COALESCE(NEW.synopsis, 'A new Telugu movie has been announced!'),
    NEW.id,
    'movies', NEW.id,
    COALESCE(NEW.poster_url, NEW.backdrop_url),
    now()
  )
  ON CONFLICT (source_table, source_id) WHERE source_table IS NOT NULL
  DO NOTHING;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_new_movie_feed
  AFTER INSERT ON movies
  FOR EACH ROW EXECUTE FUNCTION sync_new_movie_to_feed();

-- 4b. Movie enters theaters → feed item
CREATE OR REPLACE FUNCTION sync_theatrical_release_to_feed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  IF OLD.in_theaters = false AND NEW.in_theaters = true THEN
    -- Only create if no theatrical_release feed item exists for this movie
    IF NOT EXISTS (
      SELECT 1 FROM public.news_feed
      WHERE movie_id = NEW.id AND content_type = 'theatrical_release'
    ) THEN
      INSERT INTO public.news_feed (
        feed_type, content_type, title, description, movie_id,
        thumbnail_url, published_at
      ) VALUES (
        'update', 'theatrical_release',
        NEW.title || ' is now in theaters!',
        'Catch ' || NEW.title || ' on the big screen.',
        NEW.id,
        COALESCE(NEW.poster_url, NEW.backdrop_url),
        now()
      );
    END IF;
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_theatrical_release_feed
  AFTER UPDATE ON movies
  FOR EACH ROW EXECUTE FUNCTION sync_theatrical_release_to_feed();

-- 4c. OTT platform availability → feed item
CREATE OR REPLACE FUNCTION sync_ott_release_to_feed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE
  v_movie_title text;
  v_movie_poster text;
  v_platform_name text;
BEGIN
  -- Only create if no ott_release feed item exists for this movie+platform combo
  IF EXISTS (
    SELECT 1 FROM public.news_feed
    WHERE movie_id = NEW.movie_id
      AND content_type = 'ott_release'
      AND title LIKE '%' || (SELECT name FROM public.platforms WHERE id = NEW.platform_id) || '%'
  ) THEN
    RETURN NEW;
  END IF;

  SELECT title, COALESCE(poster_url, backdrop_url)
    INTO v_movie_title, v_movie_poster
    FROM public.movies WHERE id = NEW.movie_id;

  SELECT name INTO v_platform_name
    FROM public.platforms WHERE id = NEW.platform_id;

  INSERT INTO public.news_feed (
    feed_type, content_type, title, description, movie_id,
    thumbnail_url, published_at
  ) VALUES (
    'update', 'ott_release',
    v_movie_title || ' now streaming on ' || v_platform_name,
    'Watch ' || v_movie_title || ' on ' || v_platform_name || '.',
    NEW.movie_id,
    v_movie_poster,
    now()
  );
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_ott_release_feed
  AFTER INSERT ON movie_platforms
  FOR EACH ROW EXECUTE FUNCTION sync_ott_release_to_feed();

-- 4d. Rating milestone → feed item (enhances existing review trigger)
CREATE OR REPLACE FUNCTION sync_rating_milestone_to_feed()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE
  v_movie_id uuid;
  v_count integer;
  v_movie_title text;
  v_movie_poster text;
  v_milestone integer;
  v_milestones integer[] := ARRAY[10, 25, 50, 100, 250, 500];
BEGIN
  IF TG_OP = 'DELETE' THEN
    v_movie_id := OLD.movie_id;
  ELSE
    v_movie_id := NEW.movie_id;
  END IF;

  SELECT COUNT(*) INTO v_count FROM public.reviews WHERE movie_id = v_movie_id;

  -- Check if we just crossed a milestone
  FOREACH v_milestone IN ARRAY v_milestones LOOP
    IF v_count = v_milestone THEN
      SELECT title, COALESCE(poster_url, backdrop_url)
        INTO v_movie_title, v_movie_poster
        FROM public.movies WHERE id = v_movie_id;

      -- Only create if no milestone feed item exists for this count
      IF NOT EXISTS (
        SELECT 1 FROM public.news_feed
        WHERE movie_id = v_movie_id
          AND content_type = 'rating_milestone'
          AND description LIKE '%' || v_milestone || ' reviews%'
      ) THEN
        INSERT INTO public.news_feed (
          feed_type, content_type, title, description, movie_id,
          thumbnail_url, published_at
        ) VALUES (
          'update', 'rating_milestone',
          v_movie_title || ' has reached ' || v_milestone || ' reviews!',
          v_movie_title || ' now has ' || v_milestone || ' reviews from the community.',
          v_movie_id,
          v_movie_poster,
          now()
        );
      END IF;
      EXIT; -- only one milestone at a time
    END IF;
  END LOOP;

  RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE TRIGGER trg_rating_milestone_feed
  AFTER INSERT OR DELETE ON reviews
  FOR EACH ROW EXECUTE FUNCTION sync_rating_milestone_to_feed();

-- ============================================================
-- 5. ADDITIONAL INDEXES FOR SCORING PERFORMANCE
-- ============================================================
CREATE INDEX idx_news_feed_scoring ON news_feed (is_pinned DESC, published_at DESC)
  INCLUDE (movie_id, feed_type, content_type, upvote_count, downvote_count);
CREATE INDEX idx_watchlists_movie_user ON watchlists (movie_id, user_id);
CREATE INDEX idx_movie_cast_actor ON movie_cast (actor_id, movie_id);

-- ============================================================
-- 6. PERSONALIZED FEED RPC FUNCTION
-- ============================================================
CREATE OR REPLACE FUNCTION get_personalized_feed(
  p_user_id uuid DEFAULT NULL,
  p_filter text DEFAULT 'all',
  p_limit integer DEFAULT 15,
  p_offset integer DEFAULT 0
)
RETURNS TABLE (
  id uuid,
  feed_type text,
  content_type text,
  title text,
  description text,
  movie_id uuid,
  source_table text,
  source_id uuid,
  thumbnail_url text,
  youtube_id text,
  duration text,
  is_pinned boolean,
  is_featured boolean,
  display_order integer,
  published_at timestamptz,
  created_at timestamptz,
  upvote_count integer,
  downvote_count integer,
  movie_title text,
  movie_poster_url text,
  movie_release_date date,
  score numeric
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  RETURN QUERY
  WITH user_genres AS (
    SELECT UNNEST(m.genres) AS genre, COUNT(*) AS cnt
    FROM public.watchlists w
    JOIN public.movies m ON m.id = w.movie_id
    WHERE w.user_id = p_user_id AND p_user_id IS NOT NULL
    GROUP BY UNNEST(m.genres)
  ),
  user_actor_movies AS (
    SELECT DISTINCT mc.movie_id
    FROM public.favorite_actors fa
    JOIN public.movie_cast mc ON mc.actor_id = fa.actor_id
    WHERE fa.user_id = p_user_id AND p_user_id IS NOT NULL
  ),
  user_platforms AS (
    SELECT DISTINCT mp.platform_id
    FROM public.watchlists w
    JOIN public.movie_platforms mp ON mp.movie_id = w.movie_id
    WHERE w.user_id = p_user_id AND w.status = 'watched' AND p_user_id IS NOT NULL
  )
  SELECT
    nf.id, nf.feed_type, nf.content_type, nf.title, nf.description,
    nf.movie_id, nf.source_table, nf.source_id,
    nf.thumbnail_url, nf.youtube_id, nf.duration,
    nf.is_pinned, nf.is_featured, nf.display_order,
    nf.published_at, nf.created_at,
    nf.upvote_count, nf.downvote_count,
    m.title AS movie_title,
    m.poster_url AS movie_poster_url,
    m.release_date AS movie_release_date,
    (
      -- Freshness (30%): exponential decay, half-life 3 days (259200 seconds)
      (100.0 * EXP(-0.693 * EXTRACT(EPOCH FROM (now() - nf.published_at)) / 259200.0)) * 0.30
      -- Popularity (20%): movie review_count and rating
      + (CASE WHEN nf.movie_id IS NULL THEN 30
              ELSE LEAST(100, COALESCE(m.review_count, 0) * 5 + COALESCE(m.rating, 0) * 10)
         END) * 0.20
      -- Personalization (25%): watchlist, actors, genres, platforms
      + (CASE WHEN p_user_id IS NULL THEN 0 ELSE
          (CASE WHEN EXISTS (
            SELECT 1 FROM public.watchlists wl
            WHERE wl.user_id = p_user_id AND wl.movie_id = nf.movie_id
          ) THEN 40 ELSE 0 END)
          + (CASE WHEN nf.movie_id IN (SELECT uam.movie_id FROM user_actor_movies uam) THEN 30 ELSE 0 END)
          + (CASE WHEN nf.movie_id IS NOT NULL AND m.genres IS NOT NULL THEN
              LEAST(20, (SELECT COUNT(*) FROM user_genres ug WHERE ug.genre = ANY(m.genres))::integer * 5)
             ELSE 0 END)
          + (CASE WHEN EXISTS (
              SELECT 1 FROM public.movie_platforms mp2
              JOIN user_platforms up ON up.platform_id = mp2.platform_id
              WHERE mp2.movie_id = nf.movie_id
            ) THEN 10 ELSE 0 END)
         END) * 0.25
      -- Admin priority (15%): pinned, featured
      + (CASE WHEN nf.is_pinned THEN 100
              WHEN nf.is_featured THEN 70
              WHEN m.is_featured THEN 40
              ELSE 0 END) * 0.15
      -- Engagement (10%): upvotes minus downvotes
      + (LEAST(100, GREATEST(0, COALESCE(nf.upvote_count, 0) * 3 - COALESCE(nf.downvote_count, 0) * 2))) * 0.10
    )::numeric AS score
  FROM public.news_feed nf
  LEFT JOIN public.movies m ON m.id = nf.movie_id
  WHERE
    CASE p_filter
      WHEN 'all' THEN true
      WHEN 'trailers' THEN nf.feed_type = 'video' AND nf.content_type IN ('trailer', 'teaser', 'glimpse', 'promo')
      WHEN 'songs' THEN nf.content_type = 'song'
      WHEN 'posters' THEN nf.feed_type = 'poster'
      WHEN 'bts' THEN nf.content_type IN ('bts', 'interview', 'event', 'making')
      WHEN 'surprise' THEN nf.feed_type = 'surprise'
      WHEN 'updates' THEN nf.feed_type = 'update'
      ELSE true
    END
  ORDER BY nf.is_pinned DESC, score DESC, nf.published_at DESC, nf.id
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;

-- Grant execute to authenticated and anon users
GRANT EXECUTE ON FUNCTION get_personalized_feed TO authenticated, anon;

NOTIFY pgrst, 'reload schema';
